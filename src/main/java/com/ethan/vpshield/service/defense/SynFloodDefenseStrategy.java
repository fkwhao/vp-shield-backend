...
public class SynFloodDefenseStrategy implements DefenseStrategy {
    ...
    public Alert.AlertType getSupportedAttackType() {
        return Alert.AlertType.SYN_FLOOD;
    }

    @Override
    public DefenseResult execute(Alert alert, TrafficStats stats) {
        log.warn("执行 SYN Flood 防御 - 攻击源: {}", alert.getSourceIp());
        ...
        if (sourceIp != null && !sourceIp.equals("unknown")) {
            ipBlocker.block(sourceIp, "SYN Flood 攻击源", blockDuration);
            ...
            return DefenseResult.success(
                    "BLOCK_IP",
                    String.format("已封禁 SYN Flood 攻击源: %s", sourceIp),
                    sourceIp
            );
        }
